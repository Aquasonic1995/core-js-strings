...
function getCardId(value) {
  ...
  const suits = '♣♦♥♠';
  const normalizedValue = value.replace('10', 'T');
  const valueArr = normalizedValue.split('');
  const cardIndex = cards.indexOf(valueArr[0]);
  const suitIndex = suits.indexOf(valueArr[1]);
  return cardIndex + suitIndex * cards.length;
}
module.exports = {
  getStringLength,
  isString,
